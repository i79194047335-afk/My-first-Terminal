...
# Замер 2026-08-08 (BTC/USDT, 5 минут): trade_check2 отвечал "error" 11 раз
# подряд — около 4.5 минут после экспирации — и только потом отдал итог.
# Прежних 10×3 с (30 с) хватало минутному форексу, но не крипте: сделка
# уходила в "итог не получен" при живом расчёте на площадке.
SETTLE_ATTEMPTS = 40
SETTLE_INTERVAL = 10.0
