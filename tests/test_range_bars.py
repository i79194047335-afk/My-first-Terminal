...
class TestRangeBarBuilder(unittest.TestCase):
    ...
    def test_gap_tick_closes_bar_and_leaves_visible_gap(self):
        ...
        # 1.1010; тик 1.1060 отстоит от close на 50п (> R) → ГЭП.
        #
        # Бар-«точка» при гэпе НЕ закрывается плоским баром, а переоткрывается
        # от новой цены. Раньше он закрывался как есть, и на неликвидных
        # инструментах это давало частокол баров с размахом 0: у MU (250
        # сделок в сутки) 26% соседних сделок отстоят дальше R, и половина
        # истории вырождалась в плоские бары. Главное свойство гэпа при этом
        # сохранено — разрыв между барами остаётся видимым.
        ...
        self.assertGreater(abs(b.current()["open"] - closed[0]["close"]), R)

    def test_gap_after_real_bar_still_closes_it(self):
        """Гэп ПОСЛЕ набравшего размах бара по-прежнему закрывает его.

        Защита от перегиба в другую сторону: не закрывать бар при разрыве
        вообще — значит всасывать скачок и раздувать размах (тот самый баг,
        ради которого гэп-путь и появился).
        """
        b = RangeBarBuilder(R)
        # 1.1000→1.1005 — бар набрал половину R (не «точка», но и не закрыт).
        # Тик 1.1060 отстоит на 55п (> R) → бар обязан закрыться КАК ЕСТЬ.
        closed = walk(b, [1.1000, 1.1005, 1.1060])
        self.assertEqual(len(closed), 1)
        gap_bar = closed[0]
        self.assertEqual(gap_bar["close"], 1.1005)
        # Скачок НЕ всосан: размах бара остался ≈ пройденному, не 55п.
        self.assertLess(gap_bar["high"] - gap_bar["low"], R)
        self.assertEqual(b.current()["open"], 1.1060)
