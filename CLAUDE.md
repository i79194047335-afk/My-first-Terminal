...
- [feeds/orderbook.py](feeds/orderbook.py) — локальная книга заявок
  (стакан). `size=0` от биржи означает СНЯТИЕ уровня, а не нулевой объём.
  Не хранится нигде. См. «Стакан» ниже.
...
Отключаемый индикатор: гистограмма лимитных заявок справа от графика, ask
над ценой, bid под ней. Меню графика → «Стакан». **Только крипта** — у FXCM
стакана нет вовсе.

**Ничего не хранится.** Подписка у биржи живёт, только пока индикатор
включён: одна стоит ~1.1 ГБ/сутки на инструмент (замер — 16.7 обновлений
в секунду). Хаб знает, кто смотрит, и командует фиду `orderbook_sub` через
ОБРАТНЫЙ канал шины (`BusServer.command` → `BusClient(on_command=…)`).
Сам поток данных остаётся односторонним.

Ловушки, на которых уже спотыкались:
- `size=0` в дельте = СНЯТИЕ уровня. Приняв буквально, книга за минуты
  забьётся мёртвыми ценами (за минуту приходит ~4000 снятий).
- Срез режется по **±2%** от mid (`BOOK_PCT` в фиде и умолчание
  `OrderBook.snapshot`). ±0.5% покрывало лишь 45% видимой шкалы.
- Склейка уровней в корзины делается ВО ФРОНТЕ
  ([js/orderbook-overlay.js](js/orderbook-overlay.js)): шаг зависит от
  зума, и только данные в браузере дают перерисовку в одном кадре с
  графиком.

**Карточка инструмента** (кнопка ⓘ) — mark/index, 24h объём и изменение,
открытый интерес, фандинг. Обновление раз в 5 с, тип шины `ticker`. Ставка
фандинга берётся ТОЛЬКО `exchange=lighter`: API отдаёт ещё binance/bybit/
hyperliquid по тем же рынкам, и цифры расходятся втрое.
...
`pre_session_brief.py` — крон 3×/сутки (04/10/17 локального = UTC+5, см.
ограничение DeepSeek по пиковым ценам ниже)., пред-Азия /
...
нет. ### DeepSeek peak/valley pricing

Скоро вводятся пиковые часы (×2 цены). Крон НЕ должен попадать в пик.

| Часы | UTC | UTC+5 |
|---|---|---|
| Пик 1 | 01:00–04:00 | 06:00–09:00 |
| Пик 2 | 06:00–10:00 | 11:00–15:00 |

Текущий крон брифинга (UTC+5):
- **04:00** (23:00 UTC) — ✅ не пик
- **10:00** (05:00 UTC) — ✅ окно между пиками (gap 04:00–06:00 UTC)
- **17:00** (12:00 UTC) — ✅ не пик

При изменении крона — сверяться с этой таблицей. Пиковая цена ×2
применяется ко ВСЕМ биллинг-единицам DeepSeek.
...
- **Новый тип сообщения хаб→браузер — обязательно в `BROADCAST_TYPES`**
  ([index.html](index.html)), если он идёт ВСЕМ и без `requestId`. Иначе
  фильтр устаревших ответов отбрасывает его МОЛЧА: `undefined` никогда не
  равен `state.requestId`. Так уже терялись `instruments` (селектор пар жил
  на захардкоженной разметке), `orderbook` и `ticker`.
...
## In-progress: intrade.bar trading bot (ветка `feature-intrade-bot`)

Документы: [BOT_TZ.md](BOT_TZ.md) — техзадание, [INTRADE_API_MAP.md](INTRADE_API_MAP.md)
— карта API площадки (разведана HAR-записью живой сессии 2026-08-05, демо,
USD/JPY). Старый `intrade_bot_plan.md` удалён 2026-08-05: он писался до
разведки, и почти все его догадки опровергнуты (выдуманный эндпоинт
`/api/trade/open` с JSON, авторизация по cookie, отсутствие понятия о
задержке открытия).

**Status:** оболочка готова (слои 1–6, 8) и Слой 7 выполнен: 20 реальных
демо-сделок, замер задержки (медиана ~1.7 с, стабильно), формат
`trade_check2` подтверждён живьём, выплата 82% подтверждена. Работа — только
в worktree `/root/projects/terminal-bot`, в `master` не сливать.

**Границы задачи: строится ОБОЛОЧКА, стратегии в ней нет.** Направление
сделки не зашивается даже временно. Стратегия подключается позже как плагин
через интерфейс `SignalSource` (`bot/strategy/base.py`) — новым файлом в
`bot/strategy/` и строкой в конфиге, без единой правки исполнителя. Детектор
всплесков с ветки `hypothesis` не тянуть.

Бот — отдельный процесс (py3.10, пакет `bot/`, а НЕ `core/`: тот обязан
парситься на 3.7). С хабом говорит обычным WS-клиентом :8765, с площадкой —
своим HTTP-клиентом. Своя панель на :8788, свой SQLite-журнал. `hub.py`,
`index.html`, `core/*` в этой задаче не редактируются вообще — прецедент
`dashboard.html` показал, что так можно.

Что важно знать про площадку (детали — в карте API):

- **Задержка открытия ~3 с** (замеры 3559 / 2910 мс) — это не сеть,
  `balance.php` в той же сессии отвечает за 200–270 мс. Цена входа
  фиксируется в момент ОТКРЫТИЯ, а не запроса; экспирация — 60 с от
  открытия. Реальное окно: сигнал → +3 с вход → +63 с расчёт. Ключевая
  метрика журнала — `latency_ms`; замерить её с VPS на 20+ сделках нужно
  ДО написания стратегии: если на волатильности задержка растёт, минутная
  экспирация может оказаться неприменимой в принципе.
- **Cookie не нужны.** reCAPTCHA только на `/login`; все прочие вызовы
  авторизуются парой полей формы `user_id` + `user_hash`. Браузер нужен
  один раз за хешем. `user_hash` — ключ от аккаунта: в логи только первые
  6 символов, в репозиторий никогда.
- **Домены смешаны:** торговля на `intrade35.bar`, котировки на
  `intrade.bar` (без «35»). Символы тоже: в котировках со слэшем
  (`USD/JPY`), в торговых запросах без (`USDJPY`) — нужен маппинг.
- **Ответы — HTML и plain text, не JSON.** Ошибки приходят голыми строками
  (`''`, `error_time_18`, `error_time_night`) и их надо проверять ДО того,
  как считать сделку открытой. Парсинг хрупкий — сырой ответ логировать при
  каждом сбое, тесты гонять на сохранённых реальных ответах
  (`tests/fixtures/intrade/`), не на выдуманных.
- **Из `SENDING` ретрай запрещён:** ответ мог дойти, повтор даст двойную
  ставку. При неопределённости — состояние `UNKNOWN` и сверка через
  `trade_load_more2.php`. НЕ через `user_real_trade.php` — по разбору
  `/profile` (2026-08-06) это ТУМБЛЕР Реал⇄Демо без параметра: такая
  «сверка» молча переключала бы счёт, возможно на реал (§3.1 карты API).
- **Режим счёта:** тип счёта читается ТОЛЬКО парсингом `/profile`
  (`checked` на активном варианте). Перед каждой ставкой — сверка с
  `account` из конфига; расхождение — жёсткий стоп, не переключение на
  лету. Приведение счёта — только `ensure_account_mode()` (читает → сравнит
  → переключит при расхождении → перечитает). Голый тумблер в клиенте
  приватный.
- **`/profile` читается ПО КУКАМ** `user_id`+`user_hash` на сессии — в
  отличие от остальных вызовов, которые авторизуются полями формы. Без кук
  площадка отвечает JS-редиректом на главную (проверено живьём 2026-08-06);
  клиент ставит их сам (`_set_auth_cookies`, доменно — только торговый хост).
- **Из панели счёт приводится к демо** кнопкой «счёт → демо» (команда
  `ensure_demo` → `ensure_account_mode("demo")`). Только в безопасную
  сторону: тумблер без параметра инвертирует счёт, поэтому кнопки «на реал»
  в боте нет и быть не должно. В dry/shadow панель к площадке не ходит.
- **`trade_check2` (расчёт) — формат `цена_расчёта;total_close;total`**
  (подтверждён живьём 2026-08-06): первое поле — ЦЕНА, не invest_close
  (название в парсере неверно, на исход не влияет). Исход — total_close
  против ставки: больше → win, равно → refund, меньше → loss. Выплата 82%
  подтверждена живьём. `report_latency` и `bot.run latency` фильтруют по
  `--mode`, чтобы имитационные dry/shadow не разбавляли замер.
- **Задержка открытия: медиана ~1.7 с** (20 сделок, 2026-08-06), разброс
  1.1–2.1 с, стабильно. Лучше разведки (2.9–3.6 с). Точность ±1 с.
- **Дневные лимиты и серия убытков считаются ПО РЕЖИМУ и в пределах суток**
  (решено 2026-08-06). `stats_today(mode=…)` и `consecutive_losses(mode=…)`
  фильтруют по `mode` и по границе суток UTC; ограничители и панель всегда
  передают свой `config.mode`. До этого имитационные dry-сделки расходовали
  лимиты реального прогона, а вчерашние убытки тянулись в сегодняшнюю серию
  и остановили demo-прогон Слоя 7. Refund серию по-прежнему не рвёт и не
  продолжает — ставка вернулась, события не было.
- Режим по умолчанию — `dry` (к площадке не ходит вовсе). Все прогоны
  только на демо. `live` заблокирован и в этой задаче не включается.
  Kill-switch — файл `bot/STOP`. Рейт-лимиты неизвестны, площадку не
  долбить: `price_now` не чаще 1/сек, как их фронт.
- В `.gitignore`: `bot_config.json`, `bot_journal.db`, `bot/STOP`,
  HAR-файлы, куки, дампы сессий. Секреты — только в `.env`.
